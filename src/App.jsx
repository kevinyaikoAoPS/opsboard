...
function detectIdleChains(whispers) {
  // whispers: [{timestamp (ms), char_count, recipient}] sorted by time, same sender/session
  //
  // Two independent detection methods:
  //   1. FAST CHAIN  - the run is much quicker than the assistant's own normal pace.
  //   2. ROSTER SCAN - the run walks the student list in order, contacting students
  ...
  while (i < whispers.length - 4) {
    // Recipients already contacted before this run starts
    const seenBefore = new Set();
    for (let k = 0; k < i; k++) seenBefore.add(whispers[k].recipient);

    let j = i + 1;
    // Gap window widened from 1-6s to 1-10s: slower roster scrollers were being
    // cut off mid-chain by the old 6s ceiling.
    ...
    if (chainLen >= 5) {
      ...
      // Method 1: anomalously fast vs the assistant's own session median
      const isFast = sessionMedian && avgChainGap < sessionMedian * 0.5;

      // Method 2: roster scan - mostly first-time recipients, walked in order
      const chainRecips = [];
      for (let k = i; k < j; k++) if (whispers[k].recipient) chainRecips.push(whispers[k].recipient);
